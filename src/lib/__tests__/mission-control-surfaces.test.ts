...
describe('mission control surface snapshots', () => {
  ...
  it('exposes Marketing as a surface snapshot with external action and analytics guardrails', () => {
    ...
    expect(marketing?.sections[0]?.id).toBe('daily-marketing-triage')
    expect(marketing?.sections.flatMap((section) => section.cards).find((card) => card.id === 'marketing-draft-readiness')?.summary).toContain('safe internal drafts')
    expect(marketing?.sections.flatMap((section) => section.cards).find((card) => card.id === 'marketing-external-action-boundary')?.nextAction).toContain('approval packet')
